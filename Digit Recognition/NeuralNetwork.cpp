...
using namespace neural_network;
	...
	int NeuralNetwork::getTestResult(float* picture, float outputVector[10])
	{
		// ---------------------------------------------------
		...
		int result = 0;
		outputVector[0] = iterator->activation;
		for (i = 1;i < m_layersLenghts[m_totalNumberOfLayers - 1];i++)
		{
			...
			outputVector[i] = iterator->activation;
		}
		return result;
	}
	...
	void NeuralNetwork::WriteTheNetworkInFile()
	{
		ofstream weightsFile("weights.dat", ios::out | ios::binary);
		if (!weightsFile) {
			cout << "Cannot open file!" << endl;
		}
		for (int i = 0;i < m_totalNumberOfWeights;i++)
		{
			weightsFile.write((char*)&m_weightsMatrices[i], sizeof(Weight));
		}
		weightsFile.close();
		if (!weightsFile.good()) 
		{
		...
		ofstream biasesFile("neurons.dat", ios::out | ios::binary);
		if (!biasesFile) {
			cout << "Cannot open file!" << endl;
		}
		for (int i = 0;i < m_totalNumberOfNeurons;i++)
		{
			biasesFile.write((char*)&m_neuronsVectors[i], sizeof(Neuron));
		}
		biasesFile.close();
		if (!biasesFile.good())
		{
	...
	void NeuralNetwork::LoadTheNetworkFromFile()
	{
		ifstream readWeights("Resource/weights.dat", ios::out | ios::binary);
		if (!readWeights) {
			cout << "Cannot open file!" << endl;
		}
		for (int i = 0;i < m_totalNumberOfWeights;i++)
		{
			readWeights.read((char*)&m_weightsMatrices[i], sizeof(Weight));
		}	
		readWeights.close();
		if (!readWeights.good()) {
		...
		ifstream readBiases("Resource/neurons.dat", ios::out | ios::binary);
		if (!readBiases) {
			cout << "Cannot open file!" << endl;
		}
		for (int i = 0;i < m_totalNumberOfNeurons;i++)
		{
			readBiases.read((char*)&m_neuronsVectors[i], sizeof(Neuron));
		}
		readBiases.close();
		if (!readBiases.good()) {
			cout << "Error occurred at reading time!" << endl;
